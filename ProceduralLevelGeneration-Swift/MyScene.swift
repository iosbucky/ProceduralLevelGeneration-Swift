...
class MyScene: SKScene, SKPhysicsContactDelegate {
    ...
    override init(size: CGSize) {
        ...
        map.roomMinSize = CGSizeMake(2, 2)
        map.roomMaxSize = CGSizeMake(6, 6)
        map.generate()

        // Create the exit
        exit = SKSpriteNode(texture: spriteAtlas.textureNamed("exit"))
        exit.position = map.exitPoint
        exit.physicsBody = SKPhysicsBody(rectangleOfSize: CGSize(width: exit.texture!.size().width - 16, height: exit.texture!.size().height - 16))
    ...
    override func update(currentTime: NSTimeInterval) {
        ...
        // Poll the DPad
        let playerVelocity = isExitingLevel ? CGPointZero : dPad.velocity
        ...
        resolveAnimationWithID(playerAnimationID)
        
        // Move "camera" so the player is in the middle of the screen
        self.world.position = CGPoint(x:-self.player.position.x + CGRectGetMidX(self.frame), y:-self.player.position.y + CGRectGetMidY(self.frame))
    ...
    private func resolveAnimationWithID(animationID: Int) {
        ...
        var animAction = player.actionForKey(animationKey!)
        ...
        animAction = SKAction.animateWithTextures(animationFrames!, timePerFrame: 5.0/60.0, resize: true, restore: false)
        if animationID == 1 {
        ...
        player.runAction(animAction!, withKey: animationKey)
    }
    ...
    private func resolveExit() {
        ...
        // Animations
        let moveAction = SKAction.moveTo(map.exitPoint, duration: 0.5)
        let rotateAction = SKAction.rotateByAngle(CGFloat(M_PI * 2), duration: 0.5)
        let fadeAction = SKAction.fadeAlphaTo(0.5, duration: 0.5)
        let scaleAction = SKAction.scaleXTo(0.0, duration: 0.5)
        let soundAction = SKAction.playSoundFileNamed("win.wav", waitForCompletion: false)
        let blockAction = SKAction.runBlock( {
            view?.presentScene(MyScene(size: self.size), transition: SKTransition.doorsCloseVerticalWithDuration(0.5))
        })
        ...
        player.runAction(exitAnimAction)
    }
